const queries = require('./queries.js');
const deviceCatalog = require('./catalog.js');
const watch = require('./watchDevice.js');

module.exports = function() {
    // Search for available devices
    return gladys.utils.sql(queries.getDeviceByService, [ 'gpio-sensor' ])
        .then((devices) => {
            return Promise.map(devices, function(device) {
                // Check if device is awaited
                var expectedTypes = catalog.findByType(device.protocol);
                if (!expectedTypes) {
                    return Promise.reject('No sensor available for ' + device.protocol);
                } else {
                    // Gets existing related device types
                    return gladys.deviceType.getByDevice(device)
                        .then((devicetypes) => {
                            // Check if device types exist
                            var nbDeviceType = devicetypes.length;
                            var indexDeviceType = 0;

                            if (nbDeviceType > 0) {
                                // Detects only missing sensors
                                do {
                                    var deviceType = devicetypes[indexDeviceType].type;
                                    indexDeviceType++;

                                    if (expectedTypes[deviceType]) {
                                        delete expectedTypes[deviceType];
                                    }
                                } while (indexDeviceType < nbDeviceType && expectedTypes.length > 0);
                            }

                            // Device type creation
                            return Promise.map(expectedTypes, function(expectedType) {
                                expectedType.device = device.id;
                                return gladys.deviceType.create(expectedType);
                            }).then(() => {
                                return watch(device);
                            });
                        });
                }
            });
        });
};
